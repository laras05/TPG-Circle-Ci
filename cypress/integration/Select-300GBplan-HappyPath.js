...
it('Select 300GB Plan-Happy Path', function() {
    ...
    cy.get('[data-testid=cart-item-name]').contains('$65 SIM Only Super+ Plan')
    //View Plan Details (if same as the plan selected)
    cy.get('[data-testid=cart-item-PLAN-details]')
        .contains('Month to month | 300GB total Max Speed data in Oz')
    ...
    //Validate monthly pay (if same as the plan selected)
    cy.get('[data-testid=cart-item-price]').should('have.text', '$60.00')
