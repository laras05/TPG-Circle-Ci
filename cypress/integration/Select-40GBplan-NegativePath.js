...
it('Select 40GB Plan-Negative Path', function() {
    ...
    cy.get('strong')
        .should('not.contain', '$40.00')  //to validate price in sticky cart
    ...
    cy.get('[data-testid=sticky-cart-section-plan] > .StickyCartItemstyles__Content-sc-bnfv9r-0 > .StickyCartItemstyles__ItemDescription-sc-bnfv9r-8')
        .contains('$40 SIM Only Lite Plan')  //to validate plan name
    ...
    cy.get('[data-testid=cart-card-title-40-sim-only-lite-plan] > .RichText__Root-sc-1i7juze-0')
        .contains('$40 SIM Only Lite Plan')  //to validate sim only plan name/details
    ...
    cy.get('[data-testid=cart-item-name]').contains('$40 SIM Only Lite Plan')  //to validate Plan name/details
