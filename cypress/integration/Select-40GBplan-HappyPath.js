...
it('Select 40GB Plan-Happy Path', function() {
    ...
    //Validate if the price shown in sticky cart is same as the plan selected
    cy.get('[data-testid=sticky-cart-price-value-plan]').should('have.text', '$40.00')
    ...
    cy.get('[data-testid=cart-summary-title]').should('have.text', 'Your cart')

    //Validate Sim Only Plan Name (if same as the plan selected)
    cy.get('[data-testid=cart-card-title-40-sim-only-lite-plan] > .RichText__Root-sc-1i7juze-0')
        .contains('$40 SIM Only Lite Plan')

    //Validate monthly pay is same as the plan selected
    ...
    cy.get('[data-testid=cart-item-name]').contains('$40 SIM Only Lite Plan')
    //View Plan Details (if same as the plan selected)
    cy.get('[data-testid=cart-item-PLAN-details]')
        .contains('Month to month | 40GB total Max Speed data in Oz')
    ...
    cy.get('[data-testid=totalcost-disclaimer]').should('have.text', 'Total min. cost $40.00')
