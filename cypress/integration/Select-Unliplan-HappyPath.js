...
it('Select Unli Plan-Happy Path', function() {
    ...
    cy.get('strong')
        .should('have.text', '$85.00')  //to validate price in sticky cart
    ...
    cy.get('[data-testid=sticky-cart-section-plan] > .StickyCartItemstyles__Content-sc-bnfv9r-0 > .StickyCartItemstyles__ItemDescription-sc-bnfv9r-8')
        .contains('$85 SIM Only Ultra+ Plan')  //to validate plan name
    ...
    cy.get('[data-testid=cart-card-title-85-sim-only-ultra-plan] > .RichText__Root-sc-1i7juze-0')
        .contains('$85 SIM Only Ultra+ Plan')  //to validate sim only plan name/details
    cy.get('.CartCardTitlestyles__CostContent-sc-cp3pvi-6 > .Pricestyles__Container-sc-j0ygq6-0 > .Pricestyles__PriceContainer-sc-j0ygq6-1')
        .should('have.text', '$85.00')  //to validate monthly pay
    cy.get('[data-testid=price]').should('have.text', '85.00')  //to validate total cost per month
    cy.get('[data-testid=totalcost-disclaimer]')
        .should('have.text', 'Total min. cost $85.00')  //to validate total cost disclaimer
    cy.get('[data-testid=cart-card-title-button]').click()  //click arrow to view Plan Details
    cy.get('[data-testid=cart-item-name]').contains('$85 SIM Only Ultra+ Plan')  //to validate Plan name/details
    cy.get('[data-testid=cart-item-price]').should('have.text', '$85.00')  //to validate monthly pay
